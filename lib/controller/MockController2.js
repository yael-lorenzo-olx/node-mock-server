...
MockController.prototype = extend(MockController.prototype, {
	...
	_findFolder: function (path, options) {
		...
		var pathArr2 = path.split('/');


		var restPathLength = options.restPath.split('/').length + 1;
		var pathRoot = pathArr.splice(0, restPathLength).join('/');

		var pathRoot2 = pathArr2.splice(0, options.restPath.split('/').length).join('/');
		log.info('\t\tpathRoot = ' + pathRoot);
		log.info('\t\tpathRoot2 = ' + pathRoot2);
		log.info('\t\tpathArr = ' + pathArr);
		log.info('\t\tpathArr2 = ' + pathArr2);

		var pathFolder = '#' + pathArr.join('#');
		var pathFolder2 = '#' + pathArr2.join('#');
		var pathFolderArr;
		var dirs;
		var output = '';
		var i;

		log.info('\t\tpathFolder = ' + pathFolder);
		log.info('\t\tpathFolder2 = ' + pathFolder2);

        log.info('\t\tTrying to read ' + pathRoot2 + '/' + pathArr2[0]);
		//var list = this.readDir(pathRoot2 + '/' + pathArr2[0] + '/#', ['.DS_Store']);
        var list = this.readDir(pathRoot2 + '/' + pathArr2[0], ['.DS_Store']);
		...
		if (typeof list !== 'undefined' && list.length > 0) { // The first directory does not exists
			log.info('\t\tThe list is not empty');
		} else {

			dirs = this.readDir(pathRoot2, ['.DS_Store']);
			log.info('\t\tReading dires pathRoot2');

			for (i = 0; i < dirs.length; i += 1) {

			    var dirItem = dirs[i].file;
				log.info('\t\t\tDir = ' + dirItem);
				if (dirItem.startsWith('#{')) {

					placeHolder = dirs[i].file;
					log.info('\t\tFound place holder = ' + placeHolder);
					break;
				}
			}

			if (placeHolder !== '') {
                log.info('\t\tPathroot + place holder = ' + pathRoot2 + '/' + placeHolder);
				pathRoot = pathRoot2 + '/' + placeHolder;

			}
		}

        if (pathFolder === '#') {
            log.info('\t\treturn = ' + pathRoot + '/#');
            return pathRoot + '/#';
        }
